...
export default function Userprofile(props:{id:number}){
    ...
    async function getbuntsuid(){
        if(User?.relation==1){
        const res= await axios.get<{id:number}>("http://localhost:3000/buntsuletters/fromprofile/"+"1/"+String(User.id))
    ...
    return(
        ...
        <div className="m-5">
                <div className="flex w-full">
                    ...
                    <div className="flex-col ml-2">
                        ...
                        {User?.relation==1 && <div className="flex flex-col">
                                                    <div className="bg-teal-100 border border-teal-300 rounded text-center w-3/5 ml-5 mt-1">友達</div>
                                                    <Link href={"/buntsu/"+String(Profileid)}><button className="ml-5 mt-3 bg-gray-900 rounded text-white h-10 px-2">文通を開く</button></Link>
                                              </div>}
        ...
        <div  className={(isformvisible ? "block " : "hidden ")+'fixed w-full h-full  top-0  px-4 z-100 bg-white  overflow-scroll'}>
            ...
            <input ref={inputRef} name="avatar"  className='mx-auto hidden' type="file"  accept="image/*" onChange={(e)=>{console.log('changeevent');  onFileInputChange(e); }}></input>
